...
const MCP_REGISTRY_OPERATION_PREFIXES: readonly any[] = Object.freeze([
...
const INTERNAL_OPERATION_IDS_HIDDEN_FROM_MCP: any = Object.freeze(new Set<any>([
...
function printHelp() : any {
  console.log(`Usage:
  node tools/verifiers/downstream-mcp-completeness-audit.ts
  node tools/verifiers/downstream-mcp-completeness-audit.ts --allow-open-gaps
...
function riskRank(value: any = "read_only") : any {
  return RISK_RANK[String(value || "read_only")] ?? RISK_RANK.read_only;
}

function isMcpRegistryCandidate(operation: Record<string, any> = {}) : any {
  ...
  return MCP_REGISTRY_OPERATION_PREFIXES.some((prefix?: any) : any => operationId.startsWith(prefix));
}

function operationRisk(operation: Record<string, any> = {}) : any {
...
function operationTarget(operation: Record<string, any> = {}) : any {
  const controller: any = String(operation.target?.controller || "").trim();
  const method: any = String(operation.target?.method || "").trim();
  return {
    controller,
    method,
...
function mcpOutletForTool(tool: Record<string, any> = {}) : any {
...
function summarizeOperationProjection({ publicOperation, tool, sourceOperation, toolsetsById }: Record<string, any>) : any {
  const sourceTarget: any = operationTarget(sourceOperation);
  const outlet: any = publicOperation?._meta?.mcpOutlet || mcpOutletForTool(tool);
  const grantableToolsets: any = (tool.toolsets || []).filter((toolsetId?: any) : any => toolsetsById.get(toolsetId)?.grantable !== false);
  return {
...
function addFinding(findings?: any, finding?: any) : any {
  findings.push({
...
function auditCompleteness({ catalog, capabilities }: Record<string, any>) : any {
  const findings: any[] = [];
  const serverOperationsById: any = new Map<any, any>(SERVER_API_OPERATIONS.map((operation?: any) : any => [operation.id, operation]));
  const catalogOperationTools: any = catalog.tools.filter((tool?: any) : any => tool.status === "active" && tool.operationId);
  const catalogToolsById: any = new Map<any, any>(catalogOperationTools.map((tool?: any) : any => [tool.id, tool]));
  const catalogToolsByOperationId: any = new Map<any, any>();
  for (const tool of catalogOperationTools) {
    const bucket: any = catalogToolsByOperationId.get(tool.operationId) || [];
    bucket.push(tool);
    catalogToolsByOperationId.set(tool.operationId, bucket);
  }
  const publicOperationsByName: any = new Map<any, any>((capabilities.operations || []).map((operation?: any) : any => [operation.name, operation]));
  const publicOperationNames: any = new Set<any>(publicOperationsByName.keys());
  const publicOperationIds: any = new Set<any>(
    (capabilities.operations || [])
      .map((operation?: any) : any => String(operation?._meta?.operationId || "").trim())
      .filter(Boolean)
  );
  const toolsetsById: any = new Map<any, any>((catalog.toolsets || []).map((toolset?: any) : any => [toolset.id, toolset]));
  ...
  for (const publicOperation of capabilities.operations || []) {
    const tool: any = catalogToolsById.get(publicOperation.name);
    if (!tool) {
      addFinding(findings, {
    ...
    const sourceOperation: any = serverOperationsById.get(tool.operationId);
    const projection: any = summarizeOperationProjection({
      publicOperation,
      tool,
      sourceOperation,
      toolsetsById
  ...
  return {
    projections: projections.sort((left?: any, right?: any) : any =>
...
async function writeReport(reportPath?: any, report?: any) : Promise<any> {
  const relativeReportPath: any = normalizeRelativePath(reportPath);
  const absoluteReportPath: any = repoPath(relativeReportPath);
  await fs.mkdir(path.dirname(absoluteReportPath), { recursive: true });
  const absolutePathPattern: any = /(?:^|["\s])(?:\/Users\/|\/home\/|\/var\/folders\/|[A-Za-z]:\\)/u;
  const secretPattern: any = /(?:bearer\s+[a-z0-9._-]+|x-meshrix-tool-token|client_secret|private_key|password)/iu;
  const assertSafe: any = () : any => {
    const text: any = `${JSON.stringify(report, null, 2)}\n`;
...
async function main() : Promise<any> {
  ...
  const capabilities: any = await callMcpCapabilitiesList({ catalog });
  ...
  const report: Record<string, any> = {
    ...
    verifier: "tools/verifiers/downstream-mcp-completeness-audit.ts",
    ...
    summary: {
      ...
      activeCatalogOperationCount: catalog.tools.filter((tool?: any) : any => tool.status === "active" && tool.operationId).length,
      publicMcpOperationCount: capabilities.operations.length,
      outletCounts: Object.fromEntries(
        (Object.entries(capabilities.outlets || {}) as [string, any][]).map(([outlet, value]: any[]) : any => [outlet, Number(value?.operationCount || 0)])
      ),
