...
import {
  ...
  requiredArray,
  stableJson
} from "./lib/verifier-inprocess-mcp-adapter.ts";
import {
...
const MCP_SECRET_REF: any = "secret://verify/upstream-fixture/mcp-token";
const MCP_REMOTE_SECRET_REF: any = "secret://verify/upstream-fixture/mcp-remote-token";
...
const verifierDispatcher: any = createVerifierOperationDispatcher("verify-upstream-fixture-transit");
const runtimeSecretValues: any = new Set<any>();
const redactionNeedles: any = new Set<any>([repoRoot, os.homedir(), process.cwd()]);

const restToken: any = `fixture-rest-${randomBytes(18).toString("hex")}`;
const mcpToken: any = `fixture-mcp-${randomBytes(18).toString("hex")}`;
...
let publishedManifestSnapshot: any = null;
let serviceConfigured: any = false;
let currentPhase: any = "initializing";
...
async function runPhase(phase?: any, fn?: any) : Promise<any> {
  currentPhase = phase;
  try {
    return await fn();
  } catch (error: any) {
...
async function writeReport(report?: any) : Promise<any> {
  ...
  const readiness: any = createUpstreamFixtureTransitReadiness(report);
  report.summary = {
    ...(report.summary || {}),
...
async function startFixtureHttpChild() : Promise<any> {
  const child: any = spawn(process.execPath, [fixtureCliPath, "--mode", "http", "--port", "0"], {
  ...
  const url: any = await new Promise((resolve?: any, reject?: any) : any => {
...
function identityPayload(response: Record<string, any> = {}) : any {
  const direct: any = response && typeof response === "object" && !Array.isArray(response) ? response : {};
  const candidate: any = direct.structuredContent || direct;
  return candidate && typeof candidate === "object" && !Array.isArray(candidate) ? candidate : {};
}

function restJson(forwardResult: Record<string, any> = {}) : any {
  const payload: any = forwardResult?.response?.json;
  return payload && typeof payload === "object" && !Array.isArray(payload) ? payload : {};
}

function fixtureIdentityProof(payload: Record<string, any> = {}, issuedToken: any = "") : any {
  const principal: any = String(payload.principal || "").trim();
  const accountId: any = String(payload.accountId || "").trim();
...
async function stopFixtureChild() : Promise<any> {
  if (!fixtureChild) return;
  const child: any = fixtureChild;
  fixtureChild = null;
...
try {
  ...
  const started: any = await runPhase("fixture-http-startup", () : any => startFixtureHttpChild());
  fixtureChild = started.child;
  const fixtureUrl: any = started.url;
  redactionNeedles.add(fixtureUrl);

  await runPhase("secret-store-provisioning", async () : Promise<any> => {
  ...
  const mcpStdioConfig: Record<string, any> = {
    transport: "stdio",
  ...
  await runPhase("service-registration", async () : Promise<any> => {
  ...
  const restService: any = services.find((service?: any) : any => service.serviceId === UPSTREAM_FIXTURE_REST_SERVICE_ID);
  const mcpService: any = services.find((service?: any) : any => service.serviceId === UPSTREAM_FIXTURE_MCP_SERVICE_ID);
  const remoteService: any = services.find((service?: any) : any => service.serviceId === MCP_REMOTE_SERVICE_ID);
  ...
  const resolvedSecrets: any = await runPhase("secret-store-binding", async () : Promise<any> => {
    const rest: any = await resolveLocalSecretPayload({
    ...
    const mcp: any = await resolveLocalSecretPayload({
      dataDir: userDataPath,
      secretRef: MCP_SECRET_REF,
  ...
  const projectedTools: any = await runPhase("upstream-tools-projection", () : any =>
    registry.listMcpTools({ serviceId: UPSTREAM_FIXTURE_MCP_SERVICE_ID, refresh: true }));
  const directByName: any = new Map<any, any>(directTools.tools.map((tool?: any) : any => [tool.name, tool]));
  const projectedByUpstreamName: any = new Map<any, any>(
    projectedTools.items.map((tool?: any) : any => [tool._meta?.upstreamToolName, tool])
  );
  ...
  const readOnlyPayload: any = identityPayload(readOnlyCall.response);
  ...
  const remoteList: any = await runPhase("upstream-mcp-http-transport-list", () : any =>
  ...
  const restList: any = await runPhase("rest-forward-records-list", () : any => registry.forward(
  ...
  const restEcho: any = await runPhase("rest-forward-echo", () : any => registry.forward(
  ...
  const restEchoPayload: any = restJson(restEcho);
  ...
  const restIdentity: any = await runPhase("rest-forward-session-identity", () : any => registry.forward(
    { serviceId: UPSTREAM_FIXTURE_REST_SERVICE_ID, operationKey: "session-identity" },
    { scopes: ["gateway:read"] }
  ));
  assert.equal(restIdentity.ok, true);
  const restIdentityProof: any = fixtureIdentityProof(restJson(restIdentity), restToken);
  ...
  let missingReadScopeRejected: any = false;
  await runPhase("rest-forward-scope-denial", async () : Promise<any> => {
    try {
  ...
  const restPurgePending: any = await runPhase("rest-forward-destructive-guard", () : any => registry.forward(
  ...
  operationPermissionPlatform = await createOperationPermissionPlatform({
    ...
    proofSubstrate: operationProofSubstrate,
  ...
  const manifestCommitter: any = createUpstreamManifestSnapshotCommitter({
    registry,
    getBaseOperations: () : any => SERVER_API_OPERATIONS,
  ...
  await runPhase("upstream-audience-refresh", () : any => manifestCommitter.refreshAudienceProjection());
  runtimeSecretValues.add(downstreamGrant.token);

  const readOnlyPublicTool: any = `upstream.${UPSTREAM_FIXTURE_TOOL_PREFIX}.records.search`;
  const identityPublicTool: any = `upstream.${UPSTREAM_FIXTURE_TOOL_PREFIX}.session.identity`;
  const destructivePublicTool: any = `upstream.${UPSTREAM_FIXTURE_TOOL_PREFIX}.records.purge`;
  const downstreamList: any = await runPhase("downstream-tools-list", () : any => callDownstreamMcp({
  ...
  const readiness: any = await writeReport(report).catch(() : any => null);
