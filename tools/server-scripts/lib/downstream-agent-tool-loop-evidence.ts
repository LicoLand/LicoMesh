...
} from "./upstream-fixture-grant.ts";

export const DOWNSTREAM_AGENT_TOOL_LOOP_REPORT_PATH: any = "build/reports/downstream-agent-tool-loop.json";
export const DOWNSTREAM_AGENT_TOOL_LOOP_SCHEMA_VERSION: any = "v0.0.1:downstream-gateway:agent-tool-loop-report-1";
export const DOWNSTREAM_AGENT_TOOL_LOOP_VERIFIER: any = "tools/server-scripts/verify-downstream-agent-tool-loop.ts";
export const DOWNSTREAM_AGENT_TOOL_LOOP_READINESS_SOURCE: any =
  "tools/server-scripts/lib/downstream-agent-tool-loop-evidence.ts#createDownstreamAgentToolLoopReadiness";
export const DOWNSTREAM_AGENT_CLIENT_TARGETS: any = MCP_SUPPORTED_TARGETS;
export const DOWNSTREAM_AGENT_CANCELLATION_TARGET: any = DOWNSTREAM_AGENT_CLIENT_TARGETS.includes("codex")
  ? "codex"
...
export function normalizeDownstreamAgentScenario(rawTurns?: any) : any {
  if (!Array.isArray(rawTurns) || rawTurns.length === 0) {
    throw new Error("Downstream agent scenario must be a non-empty array of turns.");
  }
  const seenTurnIds: any = new Set<any>();
  const turns: any = rawTurns.map((rawTurn?: any, index?: any) : any => {
    const turn: any = rawTurn && typeof rawTurn === "object" && !Array.isArray(rawTurn) ? rawTurn : {};
    const turnId: any = String(turn.turnId || "").trim();
    const kind: any = String(turn.kind || "").trim();
    if (!turnId) {
    ...
    const toolName: any = String(turn.toolName || "").trim();
    if (kind === "tools/call" && !toolName) {
      throw new Error(`Downstream agent scenario turn ${turnId} requires toolName for tools/call.`);
    }
    const args: any = turn.arguments && typeof turn.arguments === "object" && !Array.isArray(turn.arguments)
      ? turn.arguments
      : {};
    const expect: any = turn.expect && typeof turn.expect === "object" && !Array.isArray(turn.expect)
      ? turn.expect
      : {};
    return {
      turnId,
  ...
  const missingCoreTurnIds: any = DOWNSTREAM_AGENT_CORE_TURN_IDS.filter((turnId?: any) : any => !seenTurnIds.has(turnId));
...
export function stableDownstreamAgentRefHash(value: any = "") : any {
...
export function createDownstreamAgentToolLoopReadiness(report: Record<string, any> = {}) : any {
  const record: any = asRecord(report);
  const summary: any = asRecord(record.summary);
  const evidence: any = asRecord(record.evidence);
  const scenario: any = asRecord(evidence.scenario);
  const secretStoreCredentialBinding: any = asRecord(evidence.secretStoreCredentialBinding);
  const cancellationPropagation: any = asRecord(evidence.cancellationPropagation);
  const targetRuns: any = asArray(evidence.proxyClientTargets);
  ...
  for (const target of DOWNSTREAM_AGENT_CLIENT_TARGETS) {
    const item: any = targetRuns.find((run?: any) : any => asRecord(run).target === target) || {};
    ...
    const clientProtocolProfile: any = asRecord(item.clientProtocolProfile);
    ...
    const credentialProof: any = asRecord(item.credentialProof);
    ...
    if (target === DOWNSTREAM_AGENT_CANCELLATION_TARGET) {
      const targetCancellation: any = asRecord(item.cancellationPropagation);
