...
export async function runProjectedToolWorkflows({
  ...
  await destructiveTest("upstream operations stay hidden and uncallable without an exact dynamic capability", async () : Promise<any> => {
    ...
    const listed: any = await callMcpRaw(token, { method: "tools/list", params: {} }, 1011);
    assert.equal((listed.result?.tools || []).some((tool?: any) : any => tool.name === directToolName), false);
    const called: any = await callMcpRaw(token, {
      method: "tools/call",
  ...
  await test("configured upstream operations are projected as MCP tools and execute through Operation Permission", async () : Promise<any> => {
    const token: any = await createGrant("verify-upstream-operation-tool", ["meshrix.gateway.read", "meshrix.gateway.write"]);
    const directToolName: any = `upstream.${serviceId}.echo`;
    const listed: any = await callMcpRaw(token, { method: "tools/list", params: {} }, 1013);
    const tools: any = listed.result?.tools || [];
    const directTool: any = tools.find((tool?: any) : any => tool.name === directToolName);
    ...
    const beforeAudit: any = await api("GET", "/api/operation-permission/v1/audit?limit=50");
    const beforeAuditCount: any = (beforeAudit.payload.items || [])
      .filter((item?: any) : any => item.toolId === directToolName)
      .length;
    const beforeFixtureHits: any = fixtureState.echoCount;
    const called: any = await callMcpRaw(token, {
      method: "tools/call",
    ...
    const structured: any = called.result?.structuredContent || {};
    ...
    const afterAudit: any = await api("GET", "/api/operation-permission/v1/audit?limit=50");
    const afterAuditCount: any = (afterAudit.payload.items || [])
      .filter((item?: any) : any => item.toolId === directToolName)
      .length;
  ...
  await destructiveTest("approval-required projected upstream tools create operation permission pending operations", async () : Promise<any> => {
    const token: any = await createGrant("verify-upstream-operation-tool-approval", ["meshrix.gateway.read", "meshrix.gateway.write", "meshrix.gateway.maintain"], {
      scopes: ["gateway:read", "gateway:write", "gateway:maintain"],
      maxRisk: "repair_write"
    });
    const approvalToolName: any = `upstream.${serviceId}.approval`;
    const listed: any = await callMcpRaw(token, { method: "tools/list", params: {} }, 1016);
    const listedNames: any = (listed.result?.tools || []).map((tool?: any) : any => tool.name);
    assert.equal(
      listedNames.includes(approvalToolName),
      true,
      `approval projected tool missing; visible=${listedNames.filter((name?: any) : any => name.startsWith("upstream.")).join(",")}`
    );
    const before: any = fixtureState.approvalCount;
    const pending: any = await callMcpRaw(token, {
      method: "tools/call",
      params: {
    ...
    const structured: any = pending.result?.structuredContent || {};
...
export async function runUrlAuthorityEscapeWorkflow({ test }: Record<string, any>) : Promise<any> {
  await test("upstream URL authority escape inputs are rejected before forwarding", async () : Promise<any> => {
    const rejectedConfigPaths: any[] = [
      "//escape.invalid/path",
      "\\\\escape.invalid/path",
      "/\\escape.invalid/path",
    ...
    const allowed: any = safeTargetUrl(
      { baseUrl: "https://service.example:8443" },
      { path: "/echo" },
      { query: { ok: "1" } }
    );
...
export async function runAggregateTrafficPolicyWorkflow({
  ...
  await destructiveTest("endpoint pools share the service-level aggregate traffic policy", async () : Promise<any> => {
...
export async function runEndpointPoolWorkflow({
  ...
  await test("endpoint pool opens circuit on failing upstream and shifts to healthy endpoint", async () : Promise<any> => {
    const first: any = await api("POST", "/api/gateway/v1/forward", {
    ...
    const second: any = await api("POST", "/api/gateway/v1/forward", {
      serviceId: loadBalancedServiceId,
      operationKey: "pooled-echo",
    ...
    const audit: any = await api("GET", `/api/gateway/v1/audit?serviceId=${encodeURIComponent(loadBalancedServiceId)}`);
    assert.equal(audit.status, 200);
    const endpointIds: any = audit.payload.items
      .filter((item?: any) : any => item.eventType === "upstream.forward.completed")
      .map((item?: any) : any => item.payload?.endpoint?.endpointId)
      .filter(Boolean);
