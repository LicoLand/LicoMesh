...
import { getOperationPermissionDatabasePath } from "../../packages/capabilities/src/operation-permission-core/store.ts";
...
import { assertNoLeak as assertNoSensitiveReportLeak } from "./lib/report-evidence-safety.ts";
import {
...
} from "./lib/upstream-mcp-gateway-evidence.ts";
import {
...
const REPORT_PATH: any = UPSTREAM_MCP_GATEWAY_REPORT_PATH;
const PUBLIC_TOOL_NAME: any = `upstream.${UPSTREAM_FIXTURE_TOOL_PREFIX}.records.search`;
const PUBLIC_FAILING_TOOL_NAME: any = `upstream.${UPSTREAM_FIXTURE_TOOL_PREFIX}.records.get`;
const APPROVAL_SERVICE_ID: any = verifierOpaqueServiceId("fixture-mcp-approval");
const APPROVAL_TOOL_PREFIX: any = "fixture-approval";
const APPROVAL_SECRET_REF: any = "secret://verify/upstream-mcp/approval-token";
const APPROVAL_UPSTREAM_TOOL_NAME: any = "records.purge";
const APPROVAL_PUBLIC_TOOL_NAME: any = `upstream.${APPROVAL_TOOL_PREFIX}.${APPROVAL_UPSTREAM_TOOL_NAME}`;
const APPROVAL_PROJECTED_TOOL_ID: any = `upstream.${APPROVAL_SERVICE_ID.replace(/[^A-Za-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80)}.tools-call`;
const APPROVAL_SCOPES: any[] = ["gateway:read", "gateway:write", "gateway:maintain"];

const restoreCapabilityKernelEnv: any = useIsolatedCapabilityKernelForVerifier();
const userDataPath: any = await fs.mkdtemp(path.join(os.tmpdir(), "meshrix-upstream-mcp-gateway-"));
const approvalFixtureToken: any = `fixture-approval-${randomBytes(18).toString("hex")}`;

let server: any = null;
let approvalFixture: any = null;
let approvalFixtureService: any = null;
let exitCode: any = 0;
let approvalGrantId: any = "";
let approvalCapability: any = null;
let approvalToken: any = "";
let mcpRequestId: any = 100;
...
function safeEvidence(value: Record<string, any> = {}) : any {
  return JSON.parse(JSON.stringify(value, (_key?: any, child?: any) : any => {
...
function testEvidence(name?: any) : any {
  return report.tests.find((item?: any) : any => item.name === name)?.evidence || {};
}

async function writeReport() : Promise<any> {
  report.finishedAt = new Date().toISOString();
  const approval: any = testEvidence("approval-required upstream MCP call resumes exactly once with credential binding");
  const rejected: any = testEvidence("rejected and duplicate upstream MCP resolutions have no upstream side effects");
  const expired: any = testEvidence("expired upstream MCP approval has no upstream side effects");
  const audit: any = testEvidence("upstream MCP approval lifecycle emits bound audit evidence");
  report.summary = {
    testCount: report.tests.length,
    failedCount: report.tests.filter((item?: any) : any => item.status !== "passed").length,
    reportLeakScan: true,
  ...
  let safeReport: any = safeEvidence(report);
  assertNoSensitiveReportLeak(safeReport, "upstream MCP gateway E2E report");
  const readiness: any = createUpstreamMcpGatewayReadiness(safeReport);
...
function withOperationPermissionDatabase(callback?: any) : any {
  const database: any = new Database(getOperationPermissionDatabasePath(userDataPath));
  try {
...
function approvalFixtureHitCount() : any {
  return Number(approvalFixtureService?.fixture?.state?.callCount || 0);
}

async function resolvePendingOperation(pendingOperationId?: any, resolution?: any) : Promise<any> {
  return api(
    "POST",
...
async function createApprovalPendingOperation() : Promise<any> {
  mcpRequestId += 1;
  const response: any = await mcp(approvalToken, {
    jsonrpc: "2.0",
    id: mcpRequestId,
    method: "tools/call",
    params: {
  ...
  const structured: any = response.payload.result?.structuredContent || {};
  const payload: any = structured.payload || {};
  assert.equal(
    payload.status || structured.status,
    "pending_approval",
    JSON.stringify(response.payload, null, 2)
  );
  const pendingOperationId: any = String(
...
try {
  ...
  await writeVerifierLocalUpstreamSecret({
    ...
    trackSecret: (value?: any) : any => redactionNeedles.add(String(value || "")),
    payload: { token: approvalFixtureToken }
  });
  const approvalServiceConfig: Record<string, any> = {
    serviceId: APPROVAL_SERVICE_ID,
    serviceProtocol: "mcp",
  ...
  await test("load stdio MCP upstream service from the durable manifest snapshot", async () : Promise<any> => {
    const listed: any = await api("GET", "/api/gateway/v1/external-services");
    assert.equal(listed.status, 200, JSON.stringify(listed.payload, null, 2));
    const service: any = (listed.payload.items || []).find((item?: any) : any => item.serviceId === SERVICE_ID);
  ...
  await test("downstream MCP tools/list exposes upstream MCP tool", async () : Promise<any> => {
    const listed: any = await mcp(token, {
      jsonrpc: "2.0",
      id: 1,
      method: "tools/list",
      params: {}
    ...
    assert.equal(listed.status, 200, JSON.stringify(listed.payload, null, 2));
    const tools: any = listed.payload.result.tools || [];
  ...
  await test("downstream MCP tools/call reaches upstream MCP tools/call", async () : Promise<any> => {
    const called: any = await mcp(token, {
      jsonrpc: "2.0",
      id: 2,
      method: "tools/call",
      params: {
  ...
  await test("failed upstream MCP tools/call opens service circuit", async () : Promise<any> => {
    const failed: any = await mcp(token, {
      jsonrpc: "2.0",
      id: 3,
      method: "tools/call",
      params: {
    ...
    const blocked: any = await mcp(token, {
      jsonrpc: "2.0",
      id: 4,
      method: "tools/call",
      params: {
  ...
  await test("approval-required upstream MCP call resumes exactly once with credential binding", async () : Promise<any> => {
    ...
    const pending: any = await createApprovalPendingOperation();
    ...
    const pendingList: any = await api("GET", "/api/operation-permission/v1/pending-operations?status=pending&limit=20");
    assert.equal(pendingList.status, 200, JSON.stringify(pendingList.payload, null, 2));
    assert.equal(
      (pendingList.payload.pendingOperations || []).some((item?: any) : any =>
    ...
    const approved: any = await resolvePendingOperation(pending.pendingOperationId, "approved");
    ...
    const duplicate: any = await resolvePendingOperation(pending.pendingOperationId, "approved");
  ...
  await test("rejected and duplicate upstream MCP resolutions have no upstream side effects", async () : Promise<any> => {
    approvalFixtureService.fixture.reset();
    const beforeHits: any = approvalFixtureHitCount();
    const pending: any = await createApprovalPendingOperation();
    assert.equal(approvalFixtureHitCount(), beforeHits);

    const rejected: any = await resolvePendingOperation(pending.pendingOperationId, "denied");
  ...
  await test("expired upstream MCP approval has no upstream side effects", async () : Promise<any> => {
    approvalFixtureService.fixture.reset();
    const beforeHits: any = approvalFixtureHitCount();
    const pending: any = await createApprovalPendingOperation();
    assert.equal(approvalFixtureHitCount(), beforeHits);

    const expiredAt: any = new Date(Date.now() - 1000).toISOString();
    const expiryUpdate: any = withOperationPermissionDatabase((database?: any) : any => database.prepare(`
    ...
    const expired: any = await resolvePendingOperation(pending.pendingOperationId, "approved");
    ...
    const duplicate: any = await resolvePendingOperation(pending.pendingOperationId, "denied");
    assert.equal(duplicate.status, 409, JSON.stringify(duplicate.payload, null, 2));
  ...
  await test("upstream MCP approval lifecycle emits bound audit evidence", async () : Promise<any> => {
    const gatewayAudit: any = await api(
      "GET",
    ...
    const gatewayCompleted: any = (gatewayAudit.payload.items || []).filter((item?: any) : any =>
    ...
    const permissionAudit: any = await api(
      "GET",
    ...
    const permissionItems: any = permissionAudit.payload.items || [];
    const pendingCount: any = permissionItems.filter((item?: any) : any => item.status === "pending_approval").length;
    const completedCount: any = permissionItems.filter((item?: any) : any => item.status === "ok").length;
    ...
    const storedEvidence: any = withOperationPermissionDatabase((database?: any) : any => {
      const executionRows: any = database.prepare(`
        SELECT status, COUNT(*) AS count
        FROM tool_executions
        WHERE grant_id = ? AND tool_id = ?
        GROUP BY status
      `).all(approvalGrantId, APPROVAL_PROJECTED_TOOL_ID);
      const pendingRows: any = database.prepare(`
        SELECT status, COUNT(*) AS count
  ...
  console.error(JSON.stringify(safeEvidence({
    ok: false,
    verifier: "tools/server-scripts/verify-upstream-mcp-gateway-e2e.ts",
    error: {
